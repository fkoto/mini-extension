...
MPI_Comm comm;
{
	...
	//TODO to be changed with comm get name!!!!
	if (compare_comms(comm, MPI_COMM_WORLD)){
		sprintf(msg, "\n");
	}
	else{
...
MPI_Comm comm;
{
	...
	//TODO to be changed with mpi comm name!!!
	if (compare_comms(comm, MPI_COMM_WORLD)){
		sprintf(msg, "\n");
	}
	else{
...
#else
	sprintf(msg, "%d reduce %d (of %d bytes)",llrank,count,size);
#endif
	strcat(longmsg, msg);
	
	sprintf(msg, ", type %d, root %d",np, root);
	strcat(longmsg, msg);
